...
const AddImage = () => {
    ...
    const renderRecentItem = ({ item }) => (
        <TouchableOpacity onPress={() => setSelectedImage(item.image)}>
            <Image style={{width:100, flex: 1, height: 100, margin: 1 }} source={item.image} />
        </TouchableOpacity>
    );
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.topbar}>
                <TouchableOpacity style={{ position: 'absolute', left: 18 }}>
                    <Text style={{ color: '#FFFFFF', fontSize: 18, fontWeight: 'bold' }}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <Text style={{ color: '#FFFFFF', fontSize: 18, fontWeight: 'bold', marginRight: 5 }}>Recents</Text>
                    <Image
                        source={require('../assets/image/OtherAlbums.png')}
                    />
                </TouchableOpacity>
                <TouchableOpacity style={{ position: 'absolute', right: 18 }}>
                    <Text style={{ color: '#3897F0', fontSize: 18, fontWeight: 'bold' }}>Next</Text>
                </TouchableOpacity>
            </View>

            <View style={styles.rectangle}>
                {selectedImage && <Image style={{width:'100%',height:'100%', flex: 1 }} source={selectedImage} />}
                ...
                <View style={styles.action}>
                    ...
                    <TouchableOpacity style={styles.mutiple}>
                        ...
                        />
                        <Text style={{ color: 'white', fontWeight: 'bold' }}>   SELECT MUTIPLE</Text>
                    </TouchableOpacity>
                </View>
            </View>
